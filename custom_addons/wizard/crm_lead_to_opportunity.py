...
class Lead2OpportunityPartner(models.TransientModel):
    ...
    def _action_merge(self):
        ...
        if result_opportunity and self.env.user.has_group('custom_addons.group_custom_azerbaijan_user'):
            result_opportunity.opportunity_type_id = self.opportunity_type_id
        return result_opportunity

    def _action_convert(self):
        if not self.env.user.has_group('custom_addons.group_custom_azerbaijan_user'):
